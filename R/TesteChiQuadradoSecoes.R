...
# setwd("~/LaboratorioCienciaDados/votacao_secao_2018_RO")
setwd("./AnaliseVotacao2014x2018")
...
dados_zona <- filter(dados, NR_ZONA == zona) %>%
  select(CD_MUNICIPIO, CD_MUNICIPIO, NR_ZONA, NR_SECAO, NR_LOCAL_VOTACAO,
...
for (i in 1:n_secoes) {
  ...
  bolsonaro <- filter(dados_secao, NR_VOTAVEL == num_bolsonaro)

  haddad <- filter(dados_secao, NR_VOTAVEL == num_haddad)

  demais_cand <- filter(dados_secao, !(NR_VOTAVEL == num_haddad |NR_VOTAVEL == num_bolsonaro |
  ...
  votacao_local <- rbind(votacao_local, nulos_brancos, demais_cand, bolsonaro, haddad)

  qt_votos_secao <- rbind(nulos_brancos, demais_cand, bolsonaro, haddad) %>%
...
test <- chisq.test(matriz_votacao)
